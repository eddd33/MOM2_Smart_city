...
stockage_BIG = {h: pulp.LpVariable(f"stockage_BIG_{h}", lowBound=0) for h in range(n_heures)}
...
for i in range(n_communautes):
    for h in range(n_heures):
        ...
        if h in heures_creuses:
            problem += generateur[i, h] >= 1.1 * demande[i][h] - stockage_ESS[i, h] - stockage_EV[i, h] - stockage_BIG[h]
        ...
        # Contrainte de production pour le reste des heures
            ...
            problem += generateur[i, h] >= 1.05 * demande[i][h] - stockage_ESS[i, h] - stockage_EV[i, h] - stockage_BIG[h]
...
for i in range(n_communautes):
    for h in range(n_heures):
        print(f"Communauté {i}, Heure {h}: Production générateur = {generateur[i, h].value()}, Stockage ESS = {stockage_ESS[i, h].value()}, Stockage EV = {stockage_EV[i, h].value()}")
for h in range(n_heures):
